...
*/
		...
		for ( auto i = 0 ; i < grid.size() ; ++i )
		{
			for ( auto j = 0 ; j < grid.size() ; ++j )
				max = std::max( CheckPosition( i, j ), max );
		}
	...
	int32_t CheckPosition( int32_t x, int32_t y  )
	{
		int32_t sum = 0;
		sum = std::max( sum, HorizontalCheck( x, y ) );
		sum = std::max( sum, VerticalCheck( x, y ) );
		sum = std::max( sum, DiagonalRightCheck( x, y ) );
		sum = std::max( sum, DiagonalLeftCheck( x, y ) );

		return sum;
	}
	int32_t HorizontalCheck( int32_t x, int32_t y  )
	{
		if ( !RoomHorizontal( x ) )
			return 0;
	...
	int32_t VerticalCheck( int32_t x, int32_t y  )
	{
		if ( !RoomVertical( y ) )
			return 0;
	...
	int32_t DiagonalRightCheck( int32_t x, int32_t y  )
	{
		if ( !RoomDiagonalRight( x, y ) )
			return 0;
		...
		{
			sum *= grid[y + i][x + i];
			std::cout << grid[y + i][x + i] << " * "; 
		}

		std::cout << "\nSum : " << sum << std::endl;
		return sum;
	}
	int32_t DiagonalLeftCheck( int32_t x, int32_t y  )
	{
		if ( !RoomDiagonalLeft( x, y ) )
			return 0;
	...
	bool RoomDiagonalRight( int32_t x, int32_t y )
	{
		return RoomHorizontal( x ) && RoomVertical( y );
	}
	bool RoomDiagonalLeft( int32_t x, int32_t y )
	{
		return ( ( x >= 3 ) && RoomVertical( y ) );
	}
	bool RoomHorizontal( int32_t x )
	{
		return !( ( x + 4 ) > grid[0].size() );
	}
	bool RoomVertical( int32_t y )
	{
		return !( ( y + 4 ) > grid.size() );
	}
